 ...
 */
export function convertToGoogleToolFormat(openAITools) {
    ...
    return openAITools.map((tool) => {
        if (tool.type === "function" && tool.function) {
            return {
                ...
                parameters: convertJsonSchemaToGoogleFormat(tool.function.parameters || {}),
            };
        }
 ...
 */
function convertJsonSchemaToGoogleFormat(schema) {
    if (!schema) {
 ...
 */
function convertPropertySchema(property) {
    const converted = {
        type: property.type,
    };
