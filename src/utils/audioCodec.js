 ...
 */
export function encodeAudioToPCM16(float32Data) {
 ...
 */
export function decodePCM16ToFloat32(base64PCM) {
 ...
 */
export async function resampleAudio(audioData, sourceSampleRate, targetSampleRate) {
    if (sourceSampleRate === targetSampleRate) {
        return audioData;
    }
    // Use OfflineAudioContext for high-quality browser-based resampling
    const offlineContext = new globalThis.OfflineAudioContext(1, // mono
    Math.ceil((audioData.length * targetSampleRate) / sourceSampleRate), targetSampleRate);
 ...
 */
export function resampleAudioSync(audioData, sourceSampleRate, targetSampleRate) {
    if (sourceSampleRate === targetSampleRate) {
        return audioData;
    }
