...
export class Parser {
    ...
    parsePrimary() {
        ...
        if (token.type === TokenType.LPAREN) {
            ...
            // Check if it's a space-separated tuple (e.g., "(1 2 3)")
                ...
                const canBeVectorComponent = nextToken.type === TokenType.NUMBER ||
    ...
    parseVectorOrExpression() {
        ...
        const isCustomShapeInvocation = nextToken.type === TokenType.IDENTIFIER &&
        ...
        if (canBeVectorComponent && nextToken.type !== TokenType.COMMA) {
            ...
            while (true) {
                ...
                const isCustomShape = currentToken.type === TokenType.IDENTIFIER &&
                    this.peek().type === TokenType.LBRACE;
                if ((currentToken.type === TokenType.NUMBER ||
                    currentToken.type === TokenType.IDENTIFIER ||
                    currentToken.type === TokenType.MINUS ||
    ...
    parseForLoop() {
        ...
        let fromExpr = { type: "number", value: 1 };
        ...
        if (this.current().type === TokenType.LBRACE) {
            ...
            while (this.current().type !== TokenType.RBRACE &&
                ...
                if (this.current().type === TokenType.OPTION) {
                    ...
                    const optionName = this.expectIdentifier().value;
    ...
    isStartOfNewValue() {
        ...
        return (token.precedingWhitespace &&
        ...
        while (this.current().type !== TokenType.RBRACE &&
            ...
            switch (token.type) {
                ...
                case TokenType.FOR: {
                    ...
                    const step = this.current().type === TokenType.STEP
            ...
            case TokenType.IDENTIFIER: {
                ...
                if (this.current().type === TokenType.LBRACE) {
                    ...
                    while (this.current().type !== TokenType.RBRACE &&
                        ...
                        if (this.current().type === TokenType.POSITION ||
                            ...
                            const optionName = this.current().value;
                            this.advance();
...
export function parseShapeScript(script) {
    try {
