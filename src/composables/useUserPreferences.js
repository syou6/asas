...
import { DEFAULT_REALTIME_MODEL_ID, DEFAULT_GOOGLE_LIVE_MODEL_ID, GOOGLE_LIVE_MODELS, REALTIME_MODELS, } from "../config/models";
...
const setStoredObject = (key, value) => {
    const storage = getStorage();
    storage?.setItem(key, JSON.stringify(value));
};
const initEnabledPlugins = () => {
...
const resolveStoredModelKind = (stored) => {
    if (stored === "text-rest")
        return "text-rest";
    if (stored === "voice-google-live")
...
export function useUserPreferences() {
    const storedModelKind = resolveStoredModelKind(getStoredValue(MODEL_KIND_KEY));
    const state = reactive({
        ...
        imageGenerationBackend: getStoredValue(IMAGE_GENERATION_BACKEND_KEY) ||
            "gemini",
        comfyuiModel: getStoredValue(COMFYUI_MODEL_KEY) || "flux1-schnell-fp8.safetensors",
        htmlGenerationBackend: getStoredValue(HTML_GENERATION_BACKEND_KEY) ||
            "claude",
        pluginConfigs: migrateOldConfigs(),
    });
    watch(() => state.userLanguage, (val) => {
        setStoredValue(USER_LANGUAGE_KEY, val);
    });
    watch(() => state.suppressInstructions, (val) => {
        setStoredValue(SUPPRESS_INSTRUCTIONS_KEY, String(val));
    });
    watch(() => state.roleId, (val) => {
    ...
    watch(() => state.customInstructions, (val) => {
        setStoredValue(CUSTOM_INSTRUCTIONS_KEY, val);
    });
    watch(() => state.modelId, (val) => {
        setStoredValue(MODEL_ID_KEY, val);
    });
    watch(() => state.modelKind, (val, oldVal) => {
        ...
        if (val !== oldVal) {
            if (val === "voice-realtime") {
                ...
                const isValidRealtimeModel = REALTIME_MODELS.some((m) => m.id === state.modelId);
                ...
                const isValidGoogleModel = GOOGLE_LIVE_MODELS.some((m) => m.id === state.modelId);
    ...
    watch(() => state.textModelId, (val) => {
        setStoredValue(TEXT_MODEL_ID_KEY, val);
    });
    watch(() => state.imageGenerationBackend, (val) => {
        setStoredValue(IMAGE_GENERATION_BACKEND_KEY, val);
    });
    watch(() => state.comfyuiModel, (val) => {
        setStoredValue(COMFYUI_MODEL_KEY, val);
    });
    watch(() => state.htmlGenerationBackend, (val) => {
        setStoredValue(HTML_GENERATION_BACKEND_KEY, val);
    });
    watch(() => state.enabledPlugins, (val) => {
        setStoredObject(ENABLED_PLUGINS_KEY, val);
    }, { deep: true });
    watch(() => state.pluginConfigs, (val) => {
        setStoredObject(PLUGIN_CONFIGS_KEY, val);
    }, { deep: true });
    const buildInstructions = ({ startResponse }) => {
        const role = getRole(state.roleId);
        const pluginPrompts = role.includePluginPrompts
            ? getPluginSystemPrompts(startResponse, state.enabledPlugins, state.roleId)
            : "";
    ...
    const buildTools = ({ startResponse }) => pluginTools(startResponse, state.enabledPlugins, state.roleId);
    return {
        state,
        buildInstructions,
        buildTools,
