...
export function useRealtimeSession(options) {
    ...
    const sendDataChannelMessage = (message) => {
        if (!webrtc.dc || webrtc.dc.readyState !== "open") {
            console.warn("Cannot send message because the data channel is not ready.");
            return false;
        }
    ...
    const handleMessage = (event) => {
        ...
        switch (msg.type) {
            ...
            case "response.function_call_arguments.done": {
                ...
                if (msg.truncated) {
                    console.warn(`******* Abandoning truncated tool call for ${msg.name || msg.call_id}`);
                    processedToolCalls.delete(id);
                    break;
                }
                const previousArgs = processedToolCalls.get(id);
                if (previousArgs === argStr) {
                    console.warn(`******* Skipping duplicate tool call for ${msg.name || msg.call_id}`);
                    break;
                }
    ...
    const startChat = async () => {
        ...
        try {
            ...
            console.error(err);
            stopChat();
            globalThis.alert("Failed to start voice chat. Check console for details.");
        }
        finally {
            connecting.value = false;
        }
