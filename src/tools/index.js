 ...
 */
export function getAvailablePluginsForRole(roleId) {
 ...
 */
export function isPluginAvailableInRole(pluginName, roleId) {
 ...
 */
export function isRoleCustomizable(roleId) {
    const role = getRole(roleId);
    return role?.pluginMode === "customizable";
}
export const pluginTools = (startResponse, enabledPlugins, roleId) => {
    return pluginList
        .filter((plugin) => {
...
export const getPluginSystemPrompts = (startResponse, enabledPlugins, roleId) => {
...
export const toolExecute = async (context, name, args) => {
...
export const getToolPlugin = (name) => {
...
export const getAcceptedFileTypes = () => {
    ...
    const allTypes = uploadPlugins.flatMap((plugin) => plugin.fileUpload.acceptedTypes);
    return Array.from(new Set(allTypes));
};
export const getPluginsWithConfig = (roleId) => {
...
export const getPluginConfigValue = (configs, toolName, configKey) => {
    const plugin = plugins[toolName];
    if (!plugin?.config || plugin.config.key !== configKey)
        return undefined;
    return configs[configKey] ?? plugin.config.defaultValue;
};
export const initializePluginConfigs = () => {
    const configs = {};
