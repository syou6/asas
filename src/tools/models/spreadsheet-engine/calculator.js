 ...
 */
export function calculateSheet(sheet, allSheets) {
    ...
    const getRawValue = (cell, row, col) => {
    ...
    const collectRangeValues = (range, options) => {
        let sheetData = calculated;
        let rangeRef = range;
        let isCurrentSheet = true;
    ...
    const getRangeValues = (range) => collectRangeValues(range, { numericOnly: true });
    // Helper to get raw range values including text
    const getRangeValuesRaw = (range) => collectRangeValues(range, { numericOnly: false });
 ...
 */
export function calculateWorkbook(sheets) {
