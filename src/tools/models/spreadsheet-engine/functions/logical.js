...
const notHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("NOT requires 1 argument");
    const value = context.evaluateFormula(args[0]);
    return !value || value === 0 || value === "0" || value === false;
};
const iferrorHandler = (args, context) => {
    if (args.length !== 2)
        throw new Error("IFERROR requires 2 arguments");
    try {
...
const ifnaHandler = (args, context) => {
    if (args.length !== 2)
...
const trueHandler = (args) => {
    if (args.length !== 0)
        throw new Error("TRUE requires 0 arguments");
    return true;
};
const falseHandler = (args) => {
    if (args.length !== 0)
        throw new Error("FALSE requires 0 arguments");
    return false;
};
