...
const findMatchIndex = (lookupValue, lookupArray, matchType = 1, // 1 = less than (sorted asc), 0 = exact, -1 = greater than (sorted desc)
searchMode = 1) => {
    const compare = (a, b) => {
...
const vlookupHandler = (args, context) => {
    ...
    if (!match)
        throw new Error("Invalid table array range");
...
const hlookupHandler = (args, context) => {
    if (args.length < 3 || args.length > 4) {
...
const matchHandler = (args, context) => {
...
const indexHandler = (args, context) => {
...
const xlookupHandler = (args, context) => {
