 ...
 */
import { functionRegistry, toNumber, parseCriteria, } from "../registry";
const isLetter = (char) => /[A-Z]/i.test(char);
const isCellReference = (segment) => {
    if (!segment)
        return false;
    let index = 0;
    if (segment[index] === "$")
    ...
    if (index === colStart)
        return false; // Require at least one column letter
    if (segment[index] === "$")
        index++;
    if (index >= segment.length)
        return false; // Require row digits
...
const collectNumericValues = (args, context) => {
    const values = [];
...
const sumHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("SUM requires 1 argument");
    const values = context.getRangeValues(args[0]);
    return values.reduce((sum, val) => sum + toNumber(val), 0);
};
const averageHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("AVERAGE requires 1 argument");
    const values = context.getRangeValues(args[0]);
    if (values.length === 0)
        return 0;
...
const countHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("COUNT requires 1 argument");
    const values = context.getRangeValues(args[0]);
    return values.length;
};
const medianHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("MEDIAN requires 1 argument");
...
const modeHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("MODE requires 1 argument");
...
const stdevHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("STDEV requires 1 argument");
    const values = context.getRangeValues(args[0]).map(toNumber);
    if (values.length === 0)
        return 0;
...
const varHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("VAR requires 1 argument");
    const values = context.getRangeValues(args[0]).map(toNumber);
    if (values.length === 0)
        return 0;
    const mean = values.reduce((sum, val) => sum + val, 0) / values.length;
...
const averageifHandler = (args, context) => {
    if (args.length < 2 || args.length > 3) {
