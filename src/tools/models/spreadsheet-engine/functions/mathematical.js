 ...
 */
import { functionRegistry, toNumber } from "../registry";
const roundHandler = (args, context) => {
    if (args.length !== 2)
        throw new Error("ROUND requires 2 arguments");
    const number = toNumber(context.evaluateFormula(args[0]));
    const digits = toNumber(context.evaluateFormula(args[1]));
    const multiplier = Math.pow(10, digits);
    return Math.round(number * multiplier) / multiplier;
};
const roundupHandler = (args, context) => {
    if (args.length !== 2)
        throw new Error("ROUNDUP requires 2 arguments");
    const number = toNumber(context.evaluateFormula(args[0]));
    const digits = toNumber(context.evaluateFormula(args[1]));
    const multiplier = Math.pow(10, digits);
    return Math.ceil(number * multiplier) / multiplier;
};
const rounddownHandler = (args, context) => {
    if (args.length !== 2)
        throw new Error("ROUNDDOWN requires 2 arguments");
    const number = toNumber(context.evaluateFormula(args[0]));
    const digits = toNumber(context.evaluateFormula(args[1]));
    const multiplier = Math.pow(10, digits);
    return Math.floor(number * multiplier) / multiplier;
};
const floorHandler = (args, context) => {
    if (args.length !== 2)
        throw new Error("FLOOR requires 2 arguments");
    const number = toNumber(context.evaluateFormula(args[0]));
...
const ceilingHandler = (args, context) => {
    if (args.length !== 2)
        throw new Error("CEILING requires 2 arguments");
    const number = toNumber(context.evaluateFormula(args[0]));
    const significance = toNumber(context.evaluateFormula(args[1]));
    if (significance === 0)
        return 0;
...
const absHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("ABS requires 1 argument");
    const number = toNumber(context.evaluateFormula(args[0]));
    return Math.abs(number);
};
const powerHandler = (args, context) => {
    if (args.length !== 2)
...
const sqrtHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("SQRT requires 1 argument");
    const number = toNumber(context.evaluateFormula(args[0]));
    return Math.sqrt(number);
};
const modHandler = (args, context) => {
    if (args.length !== 2)
        throw new Error("MOD requires 2 arguments");
    const number = toNumber(context.evaluateFormula(args[0]));
...
const intHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("INT requires 1 argument");
    const number = toNumber(context.evaluateFormula(args[0]));
    return Math.floor(number);
};
const truncHandler = (args, context) => {
...
const signHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("SIGN requires 1 argument");
    const number = toNumber(context.evaluateFormula(args[0]));
    return Math.sign(number);
};
const piHandler = (args) => {
    if (args.length !== 0)
        throw new Error("PI requires 0 arguments");
    return Math.PI;
};
const expHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("EXP requires 1 argument");
    const number = toNumber(context.evaluateFormula(args[0]));
    return Math.exp(number);
};
const lnHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("LN requires 1 argument");
    const number = toNumber(context.evaluateFormula(args[0]));
    return Math.log(number);
};
const logHandler = (args, context) => {
    if (args.length < 1 || args.length > 2) {
...
const log10Handler = (args, context) => {
    if (args.length !== 1)
