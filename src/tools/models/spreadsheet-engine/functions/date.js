 ...
 */
import { functionRegistry, toNumber, toString, } from "../registry";
import { dateToSerial, serialToDate } from "../date-utils";
const MS_PER_DAY = 24 * 60 * 60 * 1000;
const nowHandler = (args) => {
    if (args.length !== 0)
        throw new Error("NOW requires 0 arguments");
...
const todayHandler = (args) => {
    if (args.length !== 0)
        throw new Error("TODAY requires 0 arguments");
    const now = new Date();
    const today = new Date(Date.UTC(now.getFullYear(), now.getMonth(), now.getDate()));
    return dateToSerial(today);
};
const dateHandler = (args, context) => {
    if (args.length !== 3)
        throw new Error("DATE requires 3 arguments");
...
const timeHandler = (args, context) => {
    if (args.length !== 3)
        throw new Error("TIME requires 3 arguments");
...
const yearHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("YEAR requires 1 argument");
    const serial = toNumber(context.evaluateFormula(args[0]));
    const date = serialToDate(serial);
    return date.getUTCFullYear();
};
const monthHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("MONTH requires 1 argument");
    const serial = toNumber(context.evaluateFormula(args[0]));
    const date = serialToDate(serial);
    return date.getUTCMonth() + 1; // 1-indexed
};
const dayHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("DAY requires 1 argument");
    const serial = toNumber(context.evaluateFormula(args[0]));
    const date = serialToDate(serial);
    return date.getUTCDate();
};
const hourHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("HOUR requires 1 argument");
    const serial = toNumber(context.evaluateFormula(args[0]));
    // Get fractional part
...
const minuteHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("MINUTE requires 1 argument");
    const serial = toNumber(context.evaluateFormula(args[0]));
    const timePart = serial - Math.floor(serial);
    const totalSeconds = Math.round(timePart * 86400);
    return Math.floor((totalSeconds % 3600) / 60);
};
const secondHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("SECOND requires 1 argument");
    const serial = toNumber(context.evaluateFormula(args[0]));
    const timePart = serial - Math.floor(serial);
    const totalSeconds = Math.round(timePart * 86400);
    return totalSeconds % 60;
};
const datedifHandler = (args, context) => {
    if (args.length !== 3)
        throw new Error("DATEDIF requires 3 arguments");
