 ...
 */
const nperHandler = (args, context) => {
    if (args.length < 3 || args.length > 5) {
    ...
    const nper = Math.log((pmtWithType - fv * rate) / (pmtWithType + pv * rate)) /
        Math.log(1 + rate);
    return nper;
};
/**
 ...
 */
const rateHandler = (args, context) => {
 ...
 */
const ipmtHandler = (args, context) => {
 ...
 */
const ppmtHandler = (args, context) => {
    if (args.length < 4 || args.length > 6) {
