 ...
 * Text Functions
 */
import { functionRegistry, toString } from "../registry";
const concatenateHandler = (args, context) => {
    if (args.length === 0)
...
const concatHandler = concatenateHandler; // Alias
const leftHandler = (args, context) => {
...
const rightHandler = (args, context) => {
    if (args.length < 1 || args.length > 2) {
...
const lenHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("LEN requires 1 argument");
    const text = toString(context.evaluateFormula(args[0]));
    return text.length;
};
const upperHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("UPPER requires 1 argument");
    const text = toString(context.evaluateFormula(args[0]));
    return text.toUpperCase();
};
const lowerHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("LOWER requires 1 argument");
    const text = toString(context.evaluateFormula(args[0]));
    return text.toLowerCase();
};
const properHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("PROPER requires 1 argument");
    const text = toString(context.evaluateFormula(args[0]));
    return text
        .toLowerCase()
        .split(" ")
...
const trimHandler = (args, context) => {
    if (args.length !== 1)
        throw new Error("TRIM requires 1 argument");
    const text = toString(context.evaluateFormula(args[0]));
    // Trim leading/trailing spaces and replace multiple spaces with single space
    return text.trim().replace(/\s+/g, " ");
};
const substituteHandler = (args, context) => {
...
const replaceHandler = (args, context) => {
    if (args.length !== 4)
        throw new Error("REPLACE requires 4 arguments");
    ...
    return (oldText.substring(0, startPos) +
        newText +
        oldText.substring(startPos + numChars));
};
const findHandler = (args, context) => {
...
const searchHandler = (args, context) => {
    if (args.length < 2 || args.length > 3) {
...
const textHandler = (args, context) => {
    if (args.length !== 2)
        throw new Error("TEXT requires 2 arguments");
...
const valueHandler = (args, context) => {
    if (args.length !== 1)
...
const exactHandler = (args, context) => {
    if (args.length !== 2)
        throw new Error("EXACT requires 2 arguments");
