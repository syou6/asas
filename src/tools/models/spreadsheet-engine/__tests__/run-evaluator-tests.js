 ...
 */
import { parseFunctionArgs, evaluateFormula, } from "../evaluator";
...
function createContext(cells = {}, ranges = {}, rawRanges) {
    const context = {
        getCellValue: (ref) => cells[ref] ?? 0,
        getRangeValues: (range) => ranges[range] ?? rawRanges?.[range] ?? [],
        getRangeValuesRaw: (range) => rawRanges?.[range] ?? ranges[range] ?? [],
        evaluateFormula: (formula) => evaluateFormula(formula, context),
    };
    return context;
}
console.log("\n=== Evaluator Tests ===\n");
...
test("evaluates COUNTA function", () => {
    const ctx = createContext({}, {}, { "A1:A5": ["Laptop", "", 42, "Projector", ""] });
