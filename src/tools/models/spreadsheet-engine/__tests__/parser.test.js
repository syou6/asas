...
import { columnToIndex, indexToColumn, parseCellRef, parseRangeRef, cellRefToA1, } from "../parser";
...
describe("Parser - Range References", () => {
    ...
    test("throws on invalid range", () => {
        ...
        expect(() => parseRangeRef("invalid:range")).toThrow("Invalid cell reference");
    });
});
describe("Parser - Cell Reference to A1", () => {
    ...
    test("converts absolute references to A1", () => {
        expect(cellRefToA1({ row: 0, col: 0, absolute: { row: true, col: true } })).toBe("$A$1");
        expect(cellRefToA1({ row: 0, col: 0, absolute: { row: false, col: true } })).toBe("$A1");
        expect(cellRefToA1({ row: 0, col: 0, absolute: { row: true, col: false } })).toBe("A$1");
    });
    test("converts cross-sheet references to A1", () => {
        expect(cellRefToA1({ row: 0, col: 0, sheet: "Sheet1" })).toBe("Sheet1!A1");
        expect(cellRefToA1({ row: 1, col: 1, sheet: "My Sheet" })).toBe("'My Sheet'!B2");
    });
