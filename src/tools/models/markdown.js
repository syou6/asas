...
const toolDefinition = {
    ...
    parameters: {
        ...
        properties: {
            ...
            markdown: {
                type: "string",
                description: "The markdown content to display. Describe embedded images in the following format: ![Detailed image prompt](__too_be_replaced_image_path__). IMPORTANT: For embedded images, you MUST use the EXACT placeholder path '__too_be_replaced_image_path__'.",
            },
        },
        required: ["title", "markdown"],
    },
};
const pushMarkdown = async (context, args) => {
    ...
    if (matches.length > 0) {
        ...
        const imagePromises = matches.map(async (match, i) => {
            ...
            try {
                ...
                const result = await generateImageWithBackend(prompt, [blankImageBase64], context);
