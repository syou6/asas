 ...
 */
export async function generateImageWithBackend(prompt, images, context) {
    try {
        // Get config (can be legacy string or new object format)
        const config = context?.getPluginConfig?.("imageGenerationBackend") ||
        ...
        const comfyuiModel = backend === "comfyui"
...
export async function generateImageCommon(context, prompt, editImage) {
    try {
        ...
        const images = editImage && context.currentResult?.data?.imageData
            ? [
                context.currentResult.data.imageData.replace(/^data:image\/[^;]+;base64,/, ""),
            ]
            : [];
...
export function createUploadedImageResult(imageData, fileName, prompt) {
    return {
        toolName,
