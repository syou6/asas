import { playOthello, } from "../logic/othelloLogic";
...
const othello = async (context, args) => {
    try {
        let command;
        if (args.action === "new_game") {
            ...
            if (typeof args.row !== "number" ||
                ...
                !args.currentSide ||
                !args.playerNames) {
                throw new Error("Move action requires row, col, board, currentSide, and playerNames parameters");
            }
            command = {
                action: "move",
                row: args.row,
            ...
            if (!args.board || !args.currentSide || !args.playerNames) {
                throw new Error("Pass action requires board, currentSide, and playerNames parameters");
            }
            command = {
                action: "pass",
                board: args.board,
